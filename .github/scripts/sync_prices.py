...
資安（2026-08-12）：表格 ID 走 PRICE_SHEET_ID 環境變數（Actions secret），
不寫死在公開 repo；同步來源表只保留對外五欄，內部定價不在裡面。
"""
import csv
import os
import html
...
SHEET_ID = os.environ.get("PRICE_SHEET_ID", "").strip()
if not SHEET_ID:
    sys.exit("錯誤：環境變數 PRICE_SHEET_ID 未設定（GitHub Actions secret 或本機 env）")
