...
RETIRED = [
    ...
    # -- 2026-08-10 醫師三篇：外部審閱驗證後退掉的醫療因果宣稱 --
    ('信任的長相', '把科別寫成固定人格公式，改「決定影像方向的是診療關係，科別只是起點」'),
    ('不傳遞任何差異', '白袍在信任評分上無顯著差異，不等於不傳遞任何訊息；改「說明醫療身分，說明不了是哪一位」'),
    ('全世界公認的信任制服', 'DRESS 2026 前瞻研究未發現信任差異，白袍是身分訊號不是信任來源'),
    ('系統就散', '把一次企劃寫成必然；改「分階段補拍，只要規格留著就接得回同一套系統」'),
    ('最漏氣', '無基數的最高級加擬人俗語，改「落差最大的那一步」'),
    ('把自己交給', '照片不決定病人是否託付醫療決定，改辨識與落差的說法'),
    ('去了別家', '病人流失的直接因果沒有依據'),
]
